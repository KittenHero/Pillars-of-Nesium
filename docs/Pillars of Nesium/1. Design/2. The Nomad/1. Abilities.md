...
Wind Magic adds a layer of complexity to the Nomad's movement. It is limited by the Wind Gauge, which in a full game may increase as time goes on. Wind moves are gross movement used to enhance traversal, with fine adjustments made by basic movements. Most basic movements have a Wind version that is executed by using the basic control + the wind move button, and the same goes for basic attacks and the wind attack button.
# Template
