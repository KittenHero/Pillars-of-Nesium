...
#export (Color) var healthy = Color.blue
#export (Color) var caution = Color.aqua
...
#				pulse_tween.interpolate_property(health_over, "tint_progress", pulse_color, danger, 1.2, Tween.TRANS_SINE, Tween.EASE_IN_OUT)
#				pulse_tween.interpolate_callback(self, 0.0, "emit_signal", "pulse")
#				pulse_tween.start()
#		else:
#			health_over.tint_progress = danger
#	else:
#		pulse_tween.set_active(false)
#		if health < health_over.max_value * caution_zone:
#			health_over.tint_progress = caution
#		else:
#			health_over.tint_progress = healthy
...
func set_max_health(value):
	health_over.max_value = value
	health_over.value = value
	health_under.max_value = value
