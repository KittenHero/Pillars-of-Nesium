...
func physics_process(parent: MC, delta: float):
	...
	if not parent.is_on_floor():
		parent.push_state(parent.STATES.AIRBORNE, {"slide_jump": false})
	elif parent.frame_count - self._args["initial"] > parent.slide_duration:
...
func enter(parent: MC):
	...
	if not "slide" in self._args:
		parent.pop_state()
	self._args["initial"] = parent.frame_count
